...
class OpenCodeProvider:
    ...
    async def _execute_impl(self, prompt: str, options: dict[str, object]) -> RawResult:
        # Use `opencode run` subcommand for non-interactive execution (v1.4+).
        # The old `opencode -p <prompt>` form no longer works — `-p` is not a
        # recognised flag and causes opencode to print its help text and exit.
        # `-c` now means --continue (resume last session), NOT project directory.
        cmd = [self._bin, "run"]
        ...
        # Pass model via -m flag (supported directly on `opencode run` in v1.4+)
        if options.get("model"):
            cmd.extend(["-m", str(options["model"])])

        # Auto-approve permissions so the agent doesn't stall waiting for input
        ...
        # Message is a positional arg to `opencode run`
        ...
        # Keep MODEL env var as fallback for providers that read it directly
        if options.get("model"):
